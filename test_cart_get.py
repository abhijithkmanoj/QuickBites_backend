...
get_cart_url = f"{base_url}/api/v1/cart"
headers = {
    "Authorization": f"Bearer {access_token}",
    "Content-Type": "application/json",
    "Origin": "http://localhost:5174"  # Simulate origin
}
payload = {
...
print(f"1. Making POST request to {add_item_url}")
response = requests.post(add_item_url, json=payload, headers=headers)
print(f"POST status: {response.status_code}")
print(f"POST response: {response.text}")

print(f"\n2. Making GET request to {get_cart_url}")
get_response = requests.get(get_cart_url, headers=headers)
